...
class StripeServiceTest {
    ...
    @BeforeEach
    void setUp() {
        when(stripeConfig.getSuccessUrl()).thenReturn("http://localhost:3000/huesped/pago-exitoso");
        when(stripeConfig.getCancelUrl()).thenReturn("http://localhost:3000/huesped/pago-fallido");
    }
    ...
    void debeCrearCheckoutExitosamente() throws Exception {
        ...
        try (MockedStatic<Session> stripeStatic = mockStatic(Session.class)) {
            stripeStatic.when(() -> Session.create(any())).thenReturn(sessionMock);
    ...
    void debeLanzarExcepcionSiStripeFalla() throws Exception {
        ...
        try (MockedStatic<Session> stripeStatic = mockStatic(Session.class)) {
            ...
            stripeStatic.when(() -> Session.create(any())).thenThrow(stripeEx);
